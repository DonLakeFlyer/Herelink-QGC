...
{
    if (!isConnected()) {
        emit communicationError(QStringLiteral("Link Error"), QStringLiteral("Link %1: %2.").arg(_bluetoothConfig->name(), "Could Not Send Data - Link is Disconnected!"));
        return;
    }

    if (_socket->write(bytes) <= 0) {
        emit communicationError(QStringLiteral("Link Error"), QStringLiteral("Link %1: %2.").arg(_bluetoothConfig->name(), "Could Not Send Data - Write Failed!"));
        return;
    }
...
{
    if (!isConnected()) {
        emit communicationError(QStringLiteral("Bluetooth Link Error"), QStringLiteral("Link %1: %2.").arg(_bluetoothConfig->name(), QStringLiteral("Could Not Read Data - link is Disconnected!")));
        return;
    }

    const qint64 byteCount = _socket->bytesAvailable();
    if (byteCount <= 0) {
        emit communicationError(QStringLiteral("Bluetooth Link Error"), QStringLiteral("Link %1: %2.").arg(_bluetoothConfig->name(), QStringLiteral("Could Not Read Data - No Data Available!")));
        return;
    }

    QByteArray buffer(byteCount, Qt::Initialization::Uninitialized);
    if (_socket->read(buffer.data(), buffer.size()) < 0) {
        emit communicationError(QStringLiteral("Bluetooth Link Error"), QStringLiteral("Link %1: %2.").arg(_bluetoothConfig->name(), QStringLiteral("Could Not Read Data - Read Failed!")));
        return;
    }
...
void BluetoothLink::_discoveryFinished()
{
    if (!isConnected()) {
        _socket->disconnectFromService();
        emit communicationError(QStringLiteral("Bluetooth Link Error"), QStringLiteral("Link %1: %2.").arg(_bluetoothConfig->device().name, QStringLiteral("Could Not Locate Device!")));
    }
}
#endif
...
BluetoothConfiguration::BluetoothConfiguration(BluetoothConfiguration *copy, QObject *parent)
...
#ifdef Q_OS_IOS
    const QString uuid = settings.value("uuid", _device.uuid.toString()).toString();
    _device.uuid = QUuid(uuid);
#else
