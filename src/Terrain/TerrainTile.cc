...
QByteArray TerrainTile::serializeFromAirMapJson(const QByteArray &input)
{
    ...
    if (parseError.error != QJsonParseError::NoError) {
        qCWarning(TerrainTileLog) << Q_FUNC_INFO << "Terrain tile json doc parse error" << parseError.errorString();
        return QByteArray();
    }

    if (!document.isObject()) {
        qCWarning(TerrainTileLog) << Q_FUNC_INFO << "Terrain tile json doc is no object";
        return QByteArray();
    }
    ...
    if (!JsonHelper::validateKeys(rootObject, rootVersionKeyInfoList, errorString)) {
        qCWarning(TerrainTileLog) << Q_FUNC_INFO << "Error in reading json: " << errorString;
        return QByteArray();
    }

    if (rootObject[_jsonStatusKey].toString() != "success") {
        qCWarning(TerrainTileLog) << Q_FUNC_INFO << "Invalid terrain tile.";
        return QByteArray();
    }
    ...
    if ((swArray.count() < 2) || (neArray.count() < 2 )) {
        qCWarning(TerrainTileLog) << Q_FUNC_INFO << "Incomplete bounding location";
        return QByteArray();
    }
